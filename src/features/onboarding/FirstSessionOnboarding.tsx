...
export function FirstSessionOnboarding({ onComplete }: { onComplete: () => void }) {
  ...
  const [rankSeed, setRankSeed] = useState<{ showId: number; tier: Extract<Tier, 'S' | 'A'> } | null>(null)
  const [busy, setBusy] = useState(false)
  const selected = useMemo(() => selectedIds.map((id) => STARTER_TITLES.find((title) => title.id === id)!).filter(Boolean), [selectedIds])
  const focused = STARTER_TITLES.find((title) => title.id === focusId) ?? selected[0] ?? STARTER_TITLES[0]
  const revealTitle = STARTER_TITLES.find((title) => title.id === rankSeed?.showId) ?? selected[0] ?? focused
  const revealCards = useMemo(() => {
    if (!rankSeed) return selected.slice(0, 3)
    const others = selected.filter((title) => title.id !== rankSeed.showId).slice(0, 2)
    return [others[0], revealTitle, others[1]].filter((title): title is StarterTitle => Boolean(title))
  }, [rankSeed, revealTitle, selected])
  ...
  const persist = async (seed: { showId: number; tier: Extract<Tier, 'S' | 'A'> } | null) => {
    if (busy) return
    setBusy(true)
    const now = startedAt
    const shows: Show[] = selected.map((title, index) => ({
      ...
      overview: title.overview, genres: title.genres, rawGenres: title.rawGenres, tradition: title.tradition, vibeIds: title.vibeIds,
    ...
    try {
      await db.transaction('rw', [db.shows, db.tierAssignments], async () => {
        ...
        if (seed) {
          await db.tierAssignments.put({ showId: seed.showId, tier: seed.tier, position: 0, updatedAt: now })
        }
      })
      setRankSeed(seed)
      setFocusId(seed?.showId ?? selected[0].id)
      setPhase('reveal')
        ...
        <AnimatePresence mode="wait">
          {phase === 'pick' && (
            <motion.section key="pick" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -24 }} className="relative z-10 flex h-full flex-col">
              <header className="flex items-center justify-between px-5 pt-[max(1.2rem,env(safe-area-inset-top))]">
                <span className="text-[12px] font-black uppercase tracking-[0.3em] text-white">Loot</span>
              ...
              <div className="flex min-h-[29svh] items-end px-5 pb-5">
                <div>
                  <p className="mb-2 text-[9px] font-black uppercase tracking-[0.2em]" style={{ color: focused.accent }}>Make it yours</p>
                  <h1 className="max-w-[330px] text-[42px] font-black leading-[0.88] tracking-[-0.075em] text-balance drop-shadow-2xl">Start with three.</h1>
                  <p className="mt-2 text-[12px] font-bold text-white/56">Pick what feels like you.</p>
                </div>
              </div>
              <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-28 no-scrollbar">
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#08080a] via-[#08080a] to-transparent px-4 pb-[max(1.1rem,env(safe-area-inset-bottom))] pt-10">
                <button disabled={selected.length < 3} onClick={() => { setFocusId(selected[0].id); setPhase('rank'); navigator.vibrate?.(8) }} className="flex h-[52px] w-full items-center justify-center gap-2 rounded-[17px] bg-white text-[11px] font-black uppercase tracking-[0.14em] text-black shadow-[0_18px_42px_rgba(0,0,0,.38)] transition active:scale-[0.985] disabled:bg-white/[0.08] disabled:text-white/32">
                  {selected.length < 3 ? `${3 - selected.length} more` : 'Shape my taste'}
          ...
          {phase === 'rank' && (
            <motion.section key="rank" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.97 }} className="relative z-10 flex h-full flex-col">
              ...
              <div className="flex min-h-[33svh] items-end px-5 pb-6">
                <div><p className="mb-2 text-[9px] font-black uppercase tracking-[0.2em] text-[#f5c453]">Seed your ranking</p><h1 className="max-w-[350px] text-[39px] font-black leading-[0.9] tracking-[-0.07em] text-balance">Give one a head start.</h1><p className="mt-2 text-[11px] font-bold text-white/48">Tap S or A. Done.</p></div>
              </div>
              <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-24 no-scrollbar">
                <div className={selected.length > 3 ? 'grid grid-cols-3 gap-2' : 'grid grid-cols-3 gap-3'}>
                  {selected.map((title) => (
                    <article key={title.id} onMouseEnter={() => setFocusId(title.id)} className="min-w-0">
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-1.5">
                        <button disabled={busy} onClick={() => void persist({ showId: title.id, tier: 'S' })} className="h-10 rounded-[11px] border border-rose-300/30 bg-rose-400/15 text-[16px] font-black text-rose-300 active:scale-95 disabled:opacity-40" aria-label={`Rank ${title.name} S tier`}>S</button>
                        <button disabled={busy} onClick={() => void persist({ showId: title.id, tier: 'A' })} className="h-10 rounded-[11px] border border-orange-300/30 bg-orange-400/15 text-[16px] font-black text-orange-300 active:scale-95 disabled:opacity-40" aria-label={`Rank ${title.name} A tier`}>A</button>
                      </div>
                    </article>
                  ))}
                </div>
              </div>
              <button disabled={busy} onClick={() => void persist(null)} className="absolute inset-x-0 bottom-[max(1.1rem,env(safe-area-inset-bottom))] mx-auto w-fit px-5 py-3 text-[9px] font-black uppercase tracking-[0.16em] text-white/34 active:text-white/70 disabled:opacity-40">Rank later</button>
            </motion.section>
          )}
